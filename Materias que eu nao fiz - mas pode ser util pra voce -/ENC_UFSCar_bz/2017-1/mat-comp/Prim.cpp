...
void Prim(int *&p, Node arvore[])
{
	...
	for(int k = 0; k < num_nos ; k++)
	{
		...
		for(int i = 0 ; i < num_nos ; i++)
		{
			...
			{
				if(arvore[i].Distancia == -1 || *(p + num_nos*(Ultimo_node - 97) + 1 + i) < arvore[i].Distancia )
				{
					arvore[i].Distancia = *(p + num_nos*(Ultimo_node - 97) + 1 + i);
