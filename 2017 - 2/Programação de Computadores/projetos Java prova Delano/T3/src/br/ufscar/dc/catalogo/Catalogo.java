...
public class Catalogo {
    ...
    // relação 1(catalogo) para muitos ( midias) 
    // Midia e filhas não reconhecem catálogo, conforme dito em sala que 
    //poderia ser ignorado essa parte do enunciado
...
tenham o mesmo ano, então são ordenadas pelo título.
    ...
    public void imprimeColecaoPorTipo(int tipo) {
...
mídia. Caso duas mídias tenham o mesmo ano, então são ordenadas pelo título.
   [CD = 1, DVD = 2 e Jogo = 3]
         */ Collection<Midia> aux = new ArrayList<>();
        for (Midia c : this.midias) {
            if (c.getTipo() == tipo) {
                aux.add(c);
            }
